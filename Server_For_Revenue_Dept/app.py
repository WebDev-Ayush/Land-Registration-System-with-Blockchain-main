...
def login():
    if request.method == 'POST':
        employeeId = request.form['employeeId'].lower()  # Convert to lowercase
        password = request.form['password']

        print(f"Login attempt for employee ID: {employeeId}")
        ...
        if not user:
            ...
            user = employeesTable.find_one({"employeeId": {"$regex": f"^{employeeId}$", "$options": "i"}})
        ...
        if user and check_password_hash(user['password'], password):
            ...
            if user:
                ...
                print(f"New password hash: {generate_password_hash(password)}")
...
def addEmployee():
    ...
    if request.method == 'POST':
        employeeId = request.form['empAddress'].lower()  # Convert to lowercase
        password = request.form['password']
        fname = request.form['fname']
        lname = request.form['lname']
        revenueDeptId = request.form['revenueDeptId']

        print(f"Adding employee with ID: {employeeId}")
        print(f"Setting password: {password}")

        # Check if employee already exists
        existing_employee = employeesTable.find_one({"employeeId": employeeId})
        if existing_employee:
            return jsonify({
                'status': 0,
                "msg": "Employee with this wallet address already exists"
            })

        # Generate password hash with specific method
        ...
        try:
            ...
            print(f"Employee added to database with ID: {result.inserted_id}")
            
            # make transaction to map revenue dept id to employee address
            res = mapRevenueDeptIdToEmployee(revenueDeptId, employeeId)

            if res:
                print(f"Successfully mapped employee {employeeId} to department {revenueDeptId}")
                return jsonify({
                    'status': 1,
